...
const AccountTypeSchema = z.enum(["credit", "depository", "loan", "other"]);
...
const AccountSchema = z.object({
  plaidAccountId: z.string().describe("Plaid unique ID for the account"),
  name: z.string().describe("Name of the account"),
  mask: z
    .string()
    .describe(
      "Last 2-4 alphanumeric characters of a n account's official account number",
    ),
  officalName: z.string().describe("Offical name of the account"),
  currentBalance: z
    .number()
  ...
  itemId: z.number().describe("Item associated with the account"),
});
